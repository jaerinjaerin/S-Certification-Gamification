...
# 경로 설정
folder1 = "privacy/jsons"  # NAT_120.json 파일들이 있는 폴더
folder2 = "term/jsons"  # {Albania}.{NAT_2008}.{sq}.T&C.md 파일들이 있는 폴더

# NAT_xxx 값을 추출하는 정규식
pattern1 = r"(NAT_\d+)\.json"  # NAT_120.json 에서 NAT_120 추출
pattern2 = r"\{.*?\}\.\{(NAT_\d+)\}\.\{.*?\}\.T&C\.md"  # {Albania}.{NAT_2008}.{sq}.T&C.md 에서 NAT_2008 추출
...
nat_from_folder2 = extract_from_folder(folder2, pattern1)
