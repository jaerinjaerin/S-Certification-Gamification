...
export const processExcelBuffer = (
  ...
  try {
    ...
    if (workbook.SheetNames.length === 0) {
      return {
        success: false,
        errors: [{ line: 0, message: 'No sheets found in the file.' }],
      };
    }
    ...
    if (!sheet1) {
      return {
        success: false,
        errors: [{ line: 0, message: 'No valid sheet found in the file.' }],
      };
    }

    // 두 번째 시트는 옵션
    const sheet2Name = workbook.SheetNames[1] || null;
    ...
    console.log('extraQuestionsResult:', extraQuestionsResult);

    return {
      success:
        questionsResult.success &&
        (extraQuestionsResult ? extraQuestionsResult.success : true),
      data: {
        questions: [
          ...(questionsResult.data?.questions || []),
          ...(extraQuestionsResult?.data?.questions || []),
        ], // "추가문제" 저장
        // extraQuestions: extraQuestionsResult?.data?.questions || [], // "추가문제" 저장
        domainCode,
        languageCode,
        jobGroup,
      },
