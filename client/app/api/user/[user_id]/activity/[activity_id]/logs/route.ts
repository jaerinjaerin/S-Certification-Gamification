...
export async function POST(request: NextRequest, props: Props) {
  try {
    ...
    const campaignDomainQuizSet = await prisma.campaignDomainQuizSet.findFirst({
      where: {
        activityId: activityId,
      },
    });

    if (!campaignDomainQuizSet) {
      return NextResponse.json(
        {
          status: 404,
    ...
    const item = await prisma.userCampaignDomainLog.create({
      data: {
        ...
        userId: userId,
        completed: false,
        campaignId: campaignDomainQuizSet.campaignId,
        campaignDomainQuizSetId: campaignDomainQuizSet.id,
        domainId: campaignDomainQuizSet.domainId,
