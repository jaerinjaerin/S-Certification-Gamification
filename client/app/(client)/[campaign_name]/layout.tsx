...
export default async function CampaignLayout({ children, params }: { children: React.ReactNode; params: { campaign_name: string } }) {
  ...
  if (response.status === 404) {
    console.error("CampaignLayout Campaign not found", params.campaign_name, response);
  ...
  if (response.status != null && response.status >= 500) {
    console.error("Server error while fetching campaign", params.campaign_name, response);
  ...
  return (
    <div className="min-w-[280px] max-w-[412px] w-full h-full min-h-svh text-base">
