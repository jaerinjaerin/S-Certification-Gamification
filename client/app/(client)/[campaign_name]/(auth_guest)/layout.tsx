...
export default async function GuestLayout({ children, params: { campaign_name } }: { children: React.ReactNode; params: { campaign_name: string } }) {
  ...
  const messages = await fetch(url, { cache: "no-cache" })
    .then((res) => res.json())
    .catch((error) => console.error("get message error", locale, campaign_name, error));
